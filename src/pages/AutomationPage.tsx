...
import { useAuth, useSolBalance } from '../hooks';
...
function getStoredStrategies(): string[] {
  try {
...
function getStoredPausedStrategies(): string[] {
  try {
...
function getStoredAsset(source: string): string {
  ...
  return exchanges.Raydium?.[0] ?? 'BONK';
}

export default function AutomationPage() {
  const { walletDetails } = useAuth();
  const { balance: solBalance, loading: isLoadingBalance } = useSolBalance(walletDetails?.address);
  ...
  const isBalanceZero = (solBalance ?? 0) < MIN_BALANCE && !isLoadingBalance && !!walletDetails?.address;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-0 items-stretch">
                  ...
                  <div className="flex-1 pb-4 sm:pb-0" style={{ borderBottom: '1px solid rgba(235, 234, 250, 0.08)', borderRight: 'none' }}>
                    ...
                    <SelectField
                      ...
                      onChange={(value) => {
                        if (!isBalanceZero) {
                          setSource(value);
                          setAsset(exchanges[value as keyof typeof exchanges]?.[0] ?? 'BONK');
                        }
                      }}
                    />
                  </div>
                            ...
                            >
                              ...
                              <div
                                ...
                                style={
                                  isSelected
                                    ? {
                                        boxShadow: 'inset 0 1px 1px 0 rgba(95, 255, 215, 0.2), inset 0 0 12px 0 rgba(95, 255, 215, 0.08), inset 0 0 8px 0 rgba(95, 255, 215, 0.06), inset 0 0 20px 0 rgba(95, 255, 215, 0.12), inset 0 12px 20px 0 rgba(95, 255, 215, 0.06)',
          ...
          >
            ...
            >
              ...
              <div className="space-y-[10px]">
                {activeAutomations.length > 0 ? (
                  ...
                  <>
                    <ConditionInput label="If price drops" value="X" suffix="%" color="green" />
                    <ConditionInput label="Action:" prefix="Sell" value="Z" suffix="%" color="red" />
                    <ConditionInput label="Cooldown:" value="Y" suffix="min" />
                  </>
                )}
              </div>
            </div>
            ...
            <div className="flex justify-end mt-auto pt-3">
              <button className="font-primary font-medium text-[13px] leading-[158%] tracking-[0.02em] text-[rgba(132,141,232,0.6)] underline hover:brightness-110 cursor-pointer transition-opacity">
                Add Strategy
              </button>
            </div>
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center py-8 sm:py-12">
            ...
            >
              ...
              >
                ...
                <Corner className="bottom-0 right-0 rotate-180" color="#5fffd7" />
                <Corner className="bottom-0 left-0 -rotate-90" color="#5fffd7" />
                <svg className="w-[8px] h-[8px] sm:w-[10px] sm:h-[10px]" viewBox="0 0 10 10" fill="none">
