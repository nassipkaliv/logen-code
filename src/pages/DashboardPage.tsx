  ...
  return (
    ...
    >
      <div className="flex justify-center mb-6 sm:mb-8 md:mb-10">
        <img src={crypto.image} alt={crypto.name} className="w-16 h-16 sm:w-20 sm:h-20" />
      </div>
      ...
      <div className="space-y-2 sm:space-y-[10px]">
        ...
        <div className="flex justify-between items-center">
          ...
          <span
            ...
            style={{ background: changeBg, color: changeColor, borderLeft: `1px solid ${changeColor}` }}
          >
...
function NavArrowButton({ onClick, direction }: { onClick: () => void; direction: 'prev' | 'next' }) {
  return (
    <button
      onClick={onClick}
    ...
    >
      ...
      <svg width="6" height="10" viewBox="0 0 8 14" fill="none" className={`sm:w-2 sm:h-[14px] ${direction === 'prev' ? 'rotate-180' : ''}`}>
    ...
    const fetchCryptoData = async () => {
      try {
        ...
        const response = await fetch(
          `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=${ids}&order=market_cap_desc&per_page=${CRYPTO_COINS.length}&page=1&sparkline=false&price_change_percentage=24h`
        );
    ...
    <DashboardLayout>
      ...
      >
        ...
        <div className="relative py-3 sm:py-[15px] z-10">
          <div className="absolute top-0 left-4 right-4 sm:left-6 sm:right-6 md:left-[40px] md:right-[40px] h-[1px] bg-[rgba(235,234,250,0.08)]" />
          <NavigationArrows onPrev={() => swiperRef.current?.slidePrev()} onNext={() => swiperRef.current?.slideNext()} />
        </div>
      </div>
      ...
      <div className="relative mx-4 sm:mx-6 md:mx-[40px] py-8 sm:py-12 md:py-[60px]">
        ...
        <div className="relative flex items-center justify-center">
          <img src="/assets/img/dashboardMain.png" alt="Dashboard preview" className="w-full" />
